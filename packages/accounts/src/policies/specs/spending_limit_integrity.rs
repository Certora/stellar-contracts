...
pub fn sl_set_spending_limit_integrity(e: Env) {
    ...
    let spending_limit_data_post = get_spending_limit_data(&e, ctx_rule.id, &account_id);
    let spending_limit_post = spending_limit_data_post.spending_limit;
    cvlr_assert!(spending_limit_post == spending_limit);
}

#[rule]
// can_enforce returns false if there is no spending limit data associated with
// the smart account and context status: verified
