...
use crate::{
    ...
    smart_account::{
        specs::nondet::{nondet_context, nondet_signers_vec},
        ContextRule, Signer,
    },
};
...
pub fn wt_can_enforce_integrity(e: Env) {
    ...
    let can_enforce_result = can_enforce(&e, &context, &auth_signers, &ctx_rule, &account_id);
    clog!(threshold_pre);
    ...
    let total_weight =
        weighted_threshold::calculate_weight(&e, &auth_signers, &ctx_rule, &account_id);
    clog!(total_weight);
    clog!(threshold_pre);
