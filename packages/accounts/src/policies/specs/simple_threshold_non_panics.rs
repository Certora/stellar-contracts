...
// description: SimpleThreshold functions do not panic under appropriate
...
pub fn enforce_non_panic(
    ...
    let account_id = nondet_address();
    storage_setup_threshold(e.clone(), ctx_rule.id, account_id.clone());
    let can_enforce_result =
        can_enforce(&e, &unused_context, &authenticated_signers, &ctx_rule, &account_id);
