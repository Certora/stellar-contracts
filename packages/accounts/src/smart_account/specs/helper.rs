use cvlr::clog;
use soroban_sdk::{auth::Context, Address, Env, Vec};

use crate::{
    policies::Policy,
    smart_account::{
        specs::{policy1::Policy1, policy2::Policy2},
...
pub fn get_ids_of_rule_type(e: Env, rule_type: ContextRuleType) -> Vec<u32> {
    e.storage()
        .persistent()
        .get::<_, Vec<u32>>(&SmartAccountStorageKey::Ids(rule_type))
...
pub fn get_policies_of_id(e: Env, id: u32) -> Vec<Address> {
    e.storage()
        .persistent()
        .get::<_, Vec<Address>>(&SmartAccountStorageKey::Policies(id))
        .unwrap_or_else(|| Vec::new(&e))
}
...
pub fn get_signers_of_id(e: Env, id: u32) -> Vec<Signer> {
    e.storage()
        .persistent()
        .get::<_, Vec<Signer>>(&SmartAccountStorageKey::Signers(id))
        .unwrap_or_else(|| Vec::new(&e))
}
...
pub fn get_meta_of_id(e: Env, id: u32) -> Meta {
    e.storage()
        .persistent()
        .get::<_, Meta>(&SmartAccountStorageKey::Meta(id))
        .unwrap_or_else(|| panic!())
}

//

use crate::smart_account::{MAX_POLICIES, MAX_SIGNERS};
