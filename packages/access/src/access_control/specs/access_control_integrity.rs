...
pub fn access_control_constructor_integrity(e: Env) {
    ...
    if let Some(admin_post_internal) = &admin_post {
        clog!(admin_post_internal.as_cvlr());
    }
...
pub fn grant_role_integrity(e: Env) {
    let caller: soroban_sdk::Address = nondet_address();
    let account = nondet_address();
    let role = nondet_symbol();
    crate::access_control::grant_role(&e, &account, &role, &caller);
...
pub fn revoke_role_integrity(e: Env) {
    ...
    let account = nondet_address();
    let role = nondet_symbol();
    AccessControlContract::revoke_role(&e, account.clone(), role.clone(), caller.clone());
...
pub fn renounce_role_integrity(e: Env) {
    let caller = nondet_address();
    let role = nondet_symbol();
    AccessControlContract::renounce_role(&e, role.clone(), caller.clone());
...
pub fn set_role_admin_integrity(e: Env) {
    ...
    cvlr_assert!(
        role_admin.is_some()
            && role_admin.unwrap().to_val().get_payload() == admin_role.to_val().get_payload()
    );
}
