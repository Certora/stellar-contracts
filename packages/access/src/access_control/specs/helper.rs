use cvlr::clog;
use cvlr_soroban::SorobanAsCvlr;
use soroban_sdk::{Address, Env, Symbol};

use crate::access_control::{storage::RoleAccountKey, AccessControlStorageKey};
...
pub fn get_pending_admin(e: &Env) -> Option<Address> {
    let pending_admin =
        e.storage().temporary().get::<_, Address>(&AccessControlStorageKey::PendingAdmin);
    if let Some(pending_admin_internal) = pending_admin.clone() {
        clog!(pending_admin_internal.as_cvlr());
    }
    pending_admin
}

/// Returns `Some(Address)` if an account exists at the specified index for the
/// given role, or `None` if there is no account at that index.
///
/// # Arguments
///
/// * `e` - Access to the Soroban environment.
