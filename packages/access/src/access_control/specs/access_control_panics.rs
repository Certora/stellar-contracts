...
use cvlr_soroban::{SorobanAsCvlr, is_auth, nondet_address, nondet_symbol};
...
pub fn grant_role_panics_if_caller_not_admin_nor_admin_role(e: Env) {
    ...
    AccessControlContract::grant_role(&e, account, role.clone(), caller.clone());
    cvlr_assert!(false);
}
...
pub fn revoke_role_panics_if_caller_not_admin_nor_admin_role(e: Env) {
    ...
    AccessControlContract::revoke_role(&e, account, role.clone(), caller.clone());
    cvlr_assert!(false);
}
