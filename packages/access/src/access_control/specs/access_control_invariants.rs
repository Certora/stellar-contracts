...
use cvlr_soroban::{SorobanAsCvlr, nondet_address, nondet_symbol};
...
pub fn assume_pre_unique_indices_for_role(
    ...
    clog!(account1.as_cvlr());
    clog!(account2.as_cvlr());
...
pub fn after_grant_role_unique_indices_for_role(
    ...
    clog!(caller.as_cvlr());
    let account = nondet_address();
    clog!(account.as_cvlr());
    let role_granted = nondet_symbol();
    AccessControlContract::grant_role(&e, account, role_granted, caller);
    assert_post_unique_indices_for_role(&e, account1.clone(), account2.clone(), role.clone());
}
...
pub fn after_revoke_role_unique_indices_for_role(
    ...
    AccessControlContract::revoke_role(&e, account, role_revoked, caller);
    assert_post_unique_indices_for_role(&e, account1.clone(), account2.clone(), role.clone());
}
...
pub fn after_renounce_role_unique_indices_for_role(
    ...
    AccessControlContract::renounce_role(&e, role_renounced, caller);
    assert_post_unique_indices_for_role(&e, account1.clone(), account2.clone(), role.clone());
}
...
pub fn after_grant_role_role_count_minus_one_geq_index(e: Env, account: Address, role: Symbol) {
    ...
    clog!(account.as_cvlr());
    let role_granted = nondet_symbol();
    AccessControlContract::grant_role(&e, account_granted, role_granted, caller);
    assert_post_role_count_minus_one_geq_index(&e, account.clone(), role.clone());
}
...
pub fn assume_pre_has_role_index_implies_get_role_account(e: &Env, account: Address, role: Symbol) {
    ...
    if index.is_some() {
        ...
        if let Some(account_with_index_internal) = account_with_index.clone() {
            clog!(account_with_index_internal.as_cvlr());
        }
...
// note: still spurious after Jan 14 fix
