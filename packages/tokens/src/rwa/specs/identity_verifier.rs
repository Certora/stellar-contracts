...
use crate::rwa::{
    ...
    identity_verifier::{storage, IdentityVerifier},
};
...
#[rule]
// after set_claim_topics_and_issuers the claim_topics_and_issuers address is
// the given input status: verified
...
#[rule]
// after set_identity_registry_storage the identity_registry_storage address is
// the given input status: verified
...
#[rule]
// after recovery_target the recovery target is the same as the recovery target
// in the identity_registry_storage status: 
pub fn recovery_target_matches_identity_registry_storage(e: Env) {
    ...
    let recovery_address_from_identity_registry_storage =
        identity_registry_storage::get_recovered_to(&e, &old_account);
