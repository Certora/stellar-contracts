...
pub fn rwa_recover_balance_integrity_3(e: Env) {
    ...
    if output {
        if old_account != new_account {
            cvlr_assert!(
                balance_new_account_post == balance_old_account_pre + balance_new_account_pre
            );
        } else {
...
#[rule]
// after recover_balance the new account has the frozen tokens of the old
...
#[rule]
// after recover_balance the new account has the frozen status of the old
// account status: verified
