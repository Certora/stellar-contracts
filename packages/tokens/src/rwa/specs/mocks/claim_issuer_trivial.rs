...
use soroban_sdk::{contract, contractimpl, panic_with_error, xdr::Error, Address, Bytes, Env};
...
pub static mut IS_CLAIM_VALID_RESULT_MAP: GhostMap<(Address, u32, u32, Bytes, Bytes), bool> =
    GhostMap::UnInit;
...
impl ClaimIssuer for ClaimIssuerTrivial {
    fn is_claim_valid(
        ...
        unsafe {
            let bool: bool = IS_CLAIM_VALID_RESULT_MAP.get(&(
                identity,
                claim_topic,
                scheme,
                sig_data,
...
pub fn try_is_claim_valid(
    ...
    unsafe {
        let bool: bool =
            IS_CLAIM_VALID_RESULT_MAP.get(&(identity, claim_topic, scheme, sig_data, claim_data));
        if !bool {
