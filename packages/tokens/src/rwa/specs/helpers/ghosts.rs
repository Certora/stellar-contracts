...
// This trait is implemented for all types that can use the generic Nondet
// implementation
...
// Note: BytesN<32> is explicitly NOT implemented here, so it won't match the
// generic impl Note: Address is explicitly NOT implemented here, so it won't
// match the generic impl
...
// Note: We only implement for Address here. If you need GhostVar for other
// types, add separate impl blocks for each type to avoid overlapping impl
// conflicts.
        ...
        match self {
            Self::UnInit => {
                ...
                *self = Self::Init { k: k.clone(), v: new_v.clone() };
                new_v
            } /* notice this is stronger than what we had before - if the ghost is unset we set */
            // it and not just return nondet
            Self::Init { k: my_k, v: my_v } =>
                if k == my_k {
                    my_v.clone()
                } else {
...
// Note: We only implement for BytesN<32> here. If you need GhostMap for other
// BytesN sizes, add separate impl blocks for each size to avoid overlapping
// impl conflicts. impl <K: Clone + Eq> GhostMap<K, BytesN<32>> {
//     #[inline(never)]
