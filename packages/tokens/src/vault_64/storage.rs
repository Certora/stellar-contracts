...
use stellar_contract_utils::math::math_64::fixed_point::{muldiv, Rounding};
...
impl Vault {
    ...
    pub fn convert_to_shares(e: &Env, assets: i64) -> i64 {
    ...
    pub fn convert_to_assets(e: &Env, shares: i64) -> i64 {
    ...
    pub fn max_deposit(_e: &Env, _receiver: Address) -> i64 {
        i64::MAX
    }
    ...
    ///
    /// * refer to [`Self::convert_to_shares_with_rounding()`] errors.
    pub fn preview_deposit(e: &Env, assets: i64) -> i64 {
        Self::convert_to_shares_with_rounding(e, assets, Rounding::Floor)
    }
    ...
    /// * `e` - Access to the Soroban environment.
    /// * `receiver` - The address that would receive the vault shares.
    pub fn max_mint(_e: &Env, _receiver: Address) -> i64 {
        i64::MAX
    }
    ...
    ///
    /// * refer to [`Self::convert_to_assets_with_rounding()`] errors.
    pub fn preview_mint(e: &Env, shares: i64) -> i64 {
    ...
    ///
    /// * refer to [`Self::convert_to_assets_with_rounding()`] errors.
    pub fn max_withdraw(e: &Env, owner: Address) -> i64 {
    ...
    ///
    /// * refer to [`Self::convert_to_shares_with_rounding()`] errors.
    pub fn preview_withdraw(e: &Env, assets: i64) -> i64 {
    ...
    /// * `e` - Access to the Soroban environment.
    /// * `owner` - The address that owns the vault shares.
    pub fn max_redeem(e: &Env, owner: Address) -> i64 {
    ...
    ///
    /// * refer to [`Self::convert_to_assets_with_rounding()`] errors.
    pub fn preview_redeem(e: &Env, shares: i64) -> i64 {
        Self::convert_to_assets_with_rounding(e, shares, Rounding::Floor)
    }
    ...
    pub fn deposit(
        ...
        let shares: i64 = Self::preview_deposit(e, assets);
    ...
    /// Control pattern, or using `operator.require_auth()`.
    pub fn mint(e: &Env, shares: i64, receiver: Address, from: Address, operator: Address) -> i64 {
        let max_shares = Self::max_mint(e, receiver.clone());
        if shares > max_shares {
            panic_with_error!(e, VaultTokenError::VaultExceededMaxMint);
        }
        let assets: i64 = Self::preview_mint(e, shares);
        Self::deposit_internal(e, &receiver, assets, shares, &from, &operator);
        assets
    }
    ...
    pub fn withdraw(
        ...
        let shares: i64 = Self::preview_withdraw(e, assets);
    ...
    ///   result in overflow.
    pub fn convert_to_shares_with_rounding(e: &Env, assets: i64, rounding: Rounding) -> i64 {
        use cvlr::clog;
        clog!(assets);
        if assets < 0 {
        ...
        #[cfg(feature = "certora")]
        let ts = Self::total_supply(e);
        #[cfg(feature = "certora")]
        let y = {
            // checked_add(pow) overflows iff ts > MAX - pow
            if ts > i64::MAX - pow {
        ...
        #[cfg(feature = "certora")]
        let denominator = {
            // checked_add(1) overflows iff ta == i64::MAX
            if ta == i64::MAX {
                panic_with_error!(e, VaultTokenError::MathOverflow);
            }
            ta + 1
    ...
    /// * [`VaultTokenError::MathOverflow`] - When mathematical operations
    ///   result in overflow.
    pub fn convert_to_assets_with_rounding(e: &Env, shares: i64, rounding: Rounding) -> i64 {
        if shares < 0 {
