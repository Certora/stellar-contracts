...
use crate::vault_64::fungible_64::{emit_approve, emit_mint, emit_transfer};
use crate::vault_64::fungible_64::{
    overrides::Base, FungibleTokenError, BALANCE_EXTEND_AMOUNT, BALANCE_TTL_THRESHOLD,
};
...
impl Base {
    ...
    /// * data - `[amount: i64, live_until_ledger: u32]`
    ///
    /// # Notes
    ///
    ...
    pub fn set_allowance_munged(
        ...
        let key = StorageKeyMunged::AllowanceMunged(AllowanceKeyMunged {
            owner: owner.clone(),
    ...
    pub fn update_munged(e: &Env, from: Option<&Address>, to: Option<&Address>, amount: i64) {
        ...
        if let Some(account) = from {
            ...
            e.storage()
                .persistent()
                .set(&StorageKeyMunged::BalanceMunged(account.clone()), &from_balance);
        } else {
        ...
        if let Some(account) = to {
            ...
            e.storage()
                .persistent()
                .set(&StorageKeyMunged::BalanceMunged(account.clone()), &to_balance);
        } else {
